...
// 主节点选择，基于活跃节点
func (s *PBFTSimulator) SelectLeader(round int) *node.Node { // 选择 leader 的策略函数，基于 round 与节点状态
	active := []*node.Node{} // 创建临时切片保存所有活跃节点
	...
	//=========原为active[i].m,由于m是node_apbft.go中的变量，想要使用m只能使用node.m的方法，但是这样active[i].node.m()是无效的，因此可以在apbft.go中对m进行封装============
	sort.Slice(active, func(i, j int) bool {
		return active[i].M() > active[j].M() // 更大的M值排在前面
	})
...
func (s *PBFTSimulator) ComputeTiers() { // 计算节点的层级（高/普通/低）基于吞吐量排序
	arr := append([]*node.Node{}, s.nodes...) // 复制节点切片以便排序，不修改原切片
	...
	for i, nd := range arr {
		if i < top {
			nd.Tier = node.TierHigh // [2026-03-08]
		} else if i >= n-bottom {
			nd.Tier = node.TierLow // [2026-03-08]
		} else {
			nd.Tier = node.TierNormal // [2026-03-08]
		}
	}
}
...
func (s *PBFTSimulator) RunRound(round int, request []byte) bool {
	// ======================= 【高亮-2026-03-08】新增：每轮把 round seed 注入节点（让恶意行为/随机行为可复现） =======================
