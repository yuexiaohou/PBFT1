...
export default function History() {
    ...
    //使用 finally 是 JavaScript 里的最佳实践，它能保证无论发生什么异常，最终一定会执行关闭 loading 的操作。如果不用 finally，一旦接口报错，页面可能[...]
    //if (res.data && res.data.records) { setRecords(...) } 相较于if (res.data && res.data.records) {}增加了一层安全校验（防御性编程）。只有明确确认 res.data 存在，且 res[...]
    useEffect(() => {
    ...
    // 通过 map() 方法，我们遍历 records 数组，为每条记录创建一个新的对象，其中 x 是时间戳，y 是价格，label 是卖出节点信息（如果有的话）。这样就[...]
    const chartData = useMemo(() => {
        // 后端记录是按时间倒序的，为了图表从左到右显示时间正序，我们先 reverse()
        return [...records]
            .reverse()
            // ====== 【高亮-2026-03-19】优化1：过滤掉充值记录（只展示有真实价格的交易），防止Y轴被拉低到0 ======
            .filter(r => r.price > 0)
            .map((r, i) => {
                ...
                return {
                    id: i,
                    // ====== 【高亮-2026-03-19】优化2：直接传入 Date 对象，满足 x-charts 'time' 刻度类型的严格要求 ======
                    x: new Date(safeTimeStr),
                    y: r.price, // Y轴为价格
    ...
    // ScatterChart 默认处理数字类型的 X 和 Y 轴数据。由于后端返回的时间是字符串格式，我们需要在前端将其转换为 JavaScript 的 Date 对象，并进一步转换[...]
    // 通过 valueFormatter，我们可以自定义 X 轴的显示格式，将时间戳转换回可读的时间字符串，这样用户在查看图表时就能直观地理解每个数据点对应的[...]
    // 这种处理方式不仅符合 ScatterChart 的数据格式要求，还能提升用户体验，让图表更具可读性和交互性。
    // 通过在 ScatterChart 的 series 中添加 valueFormatter，我们可以为每个数据点提供一个详细的提示信息，当用户点击或悬停在数据点上时，能够看到对应的[...]
    // 通过在 Table 中展示卖出节点信息，我们可以让用户在查看交易记录时直接看到每笔交易的卖出节点（如果有的话）。这不仅增加了表格的可读性，[...]
        ...
        <Box sx={{ p: 3, maxWidth: 1000, mx: "auto" }}>
            ...
            <Paper sx={{ p: 3, mb: 4 }}>
                ...
                {loading ? (
                        ...
                        series={[
                            {
                                label: '成交价',
                                data: chartData,
                                // ====== 【高亮-2026-03-19】优化3：悬浮提示 (Tooltip) 显示完整的日期和时间 ======
                                valueFormatter: (v) => `时间: ${v.x.toLocaleString()}, 价格: ${v.y.toFixed(2)}, 节点: ${v.label}`,
                            }
                        ]}
                        xAxis={[
                            {
                                // ====== 【高亮-2026-03-19】修复图表报错：删除 dataKey: 'x'，让组件自动读取 series.data 里的 x ======
                                scaleType: 'time', // 指定横轴刻度类型为时间
                                // ====== 【高亮-2026-03-19】优化4：X轴刻度显示完整的日期和时间 (toLocaleString) ======
                                valueFormatter: (v) => v instanceof Date ? v.toLocaleString() : new Date(v).toLocaleString(),
                                label: '交易时间' // X轴文字
                            }
                        ]}
                        yAxis={[
                    ...
                    <Typography color="text.secondary">暂无成交数据</Typography>
                )}
            </Paper>
